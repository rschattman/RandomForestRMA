---
title: "PA trial #2 - averaged weather stations"
...
# eliminate station data, maintain averaged monthly data
head(PAbeta_wide)
PAbeta_wide <- PAbeta_wide[,c("Year", "WetDollars", "WetAcres", "DryDollars", "DryAcres", "JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC")]
```

# Create new data frames with one dependent variable
```{r}
#head(PAbeta_wide)
WetAcres <- PAbeta_wide[,c(1,3,6:17)]  #subset year, dependent variable, and all precip columes
WetDollars <- PAbeta_wide[,c(1,2,6:17)]
DryAcres <- PAbeta_wide[,c(1,5,6:17)]
DryDollars <- PAbeta_wide[,c(1,4,6:17)]
```
...
## Wet Acres
```{r}
Mod1 <- randomForest(Wettrain$WetAcres~ ., 
                     data = Wettrain, 
                     ntree = 500, 
                     #method = "anova", 
                     importance = TRUE)
...
plot(res)
res$importance
varImpPlot(res, main = "Important variables for PA Wet Acres")      # Need this for pub
MDSplot(res, fac = Wettrain$WetAcre, k=2, palette = NULL, pch = 20) # Need this for pub
```

```{r}
# using randomForestExplainer
min_depth_frame_wetacres <- min_depth_distribution(res)
plot_min_depth_distribution(min_depth_frame_wetacres, mean_sample = "relevant_trees")
importance_frame_wetacres <- measure_importance(res) 
print(importance_frame_wetacres)

#Use ggpairs to determine which informative information from the importance frame
plot_importance_ggpairs(importance_frame_wetacres) #the two LEAST negative correlations in top row are the winners (?)

# pvalue used to determine whether the actual number of splits using this variable exceeds what would be expect if the splits were random.
plot_multi_way_importance(importance_frame_wetacres, x_measure = "mse_increase", y_measure = 
                            "node_purity_increase", size_measure = "p_value", no_of_labels = 5, main = "PA Wet Acres") #top 5 variables are labeled


EF_wetacres <- explain_forest(res, interactions = FALSE, data = Wettest)
...
plot(res2)
res2$importance
varImpPlot(res2, main = "Important variables for PA Wet Dollars")  
```


```{r}
# using randomForestExplainer
min_depth_frame_wetdollars <- min_depth_distribution(res2)
plot_min_depth_distribution(min_depth_frame_wetdollars, mean_sample = "relevant_trees")
importance_frame_wetdollars <- measure_importance(res2) 
print(importance_frame_wetdollars)

#Use ggpairs to determine which informative information from the importance frame
plot_importance_ggpairs(importance_frame_wetdollars) #the two LEAST negative correlations in top row are the winners (?)

# pvalue used to determine whether the actual number of splits using this variable exceeds what would be expect if the splits were random.
plot_multi_way_importance(importance_frame_wetdollars, x_measure = "mse_increase", y_measure = 
                            "node_purity_increase", size_measure = "p_value", no_of_labels = 5, main = "PA Wet Dollars") #top 5 variables are labeled


EF_wetdollars <- explain_forest(res2, interactions = FALSE, data = Wettrain2)
...
res3$importance
varImpPlot(res3, main = "Important variables for PA Dry Acres")                      
```

```{r}
# using randomForestExplainer
min_depth_frame_dryacres <- min_depth_distribution(res3)
plot_min_depth_distribution(min_depth_frame_dryacres, mean_sample = "relevant_trees")
importance_frame_dryacres <- measure_importance(res3) 
print(importance_frame_dryacres)

#Use ggpairs to determine which informative information from the importance frame
plot_importance_ggpairs(importance_frame_dryacres) #the two LEAST negative correlations in top row are the winners (?)

# pvalue used to determine whether the actual number of splits using this variable exceeds what would be expect if the splits were random.
plot_multi_way_importance(importance_frame_dryacres, x_measure = "mse_increase", y_measure = 
                            "node_purity_increase", size_measure = "p_value", no_of_labels = 5, main = "PA Dry Acres") #top 5 variables are labeled

EF_dryacres <- explain_forest(res3, interactions = FALSE, data = Drytrain)
```
...
res4$importance
varImpPlot(res4, main = "PA Dry Dollars")                      
```
```{r}
# using randomForestExplainer
min_depth_frame_drydollars <- min_depth_distribution(res4)
plot_min_depth_distribution(min_depth_frame_drydollars, mean_sample = "relevant_trees")
importance_frame_drydollars <- measure_importance(res4) 
print(importance_frame_drydollars)

#Use ggpairs to determine which informative information from the importance frame
plot_importance_ggpairs(importance_frame_drydollars) #the two LEAST negative correlations in top row are the winners (?)

# pvalue used to determine whether the actual number of splits using this variable exceeds what would be expect if the splits were random.
plot_multi_way_importance(importance_frame_drydollars, x_measure = "mse_increase", y_measure = 
                            "node_purity_increase", size_measure = "p_value", no_of_labels = 5, main = "PA Dry Acres") #top 5 variables are labeled

EF_drydollars <- explain_forest(res4, interactions = FALSE, data = Drytrain2)
```
